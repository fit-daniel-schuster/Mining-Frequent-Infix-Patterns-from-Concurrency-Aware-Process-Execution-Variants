...
import pandas as pd
import timeit
from cortado_core.experiments.subpattern_eval.Algos.asai_performance import min_sub_mining_asai_memory
from cortado_core.experiments.subpattern_eval.Algos.valid_performance import min_sub_mining_memory
from cortado_core.experiments.subpattern_eval.exit_after import run_mining_memory_eval

from cortado_core.subprocess_discovery.subtree_mining.maximal_connected_components.maximal_connected_check import check_if_valid_tree, set_maximaly_closed_patterns
...
from cortado_core.subprocess_discovery.subtree_mining.utilities import compute_occurence_list_size
from cortado_core.utils.cvariants import get_concurrency_variants
from pm4py.objects.log.importer.xes.importer import apply as xes_import


def compare_occurence_list_size(log, log_name, strategy, strategy_name, timeout, artStart): 
    ...
    for rel_sup in min_sups:
        ...
        for algo, prms, name in performanceTest:
            ...
            if not name in bailouted:
                rTimes, k_patterns, nC, bailOut = run_mining_memory_eval(algo, timeout, prms = prms)
            ...
            for k, patterns in k_patterns.items(): 
                try:
                    occ_lists.append(sum([compute_occurence_list_size(f.rmo) for f in patterns]))
            ...
            print('OccList' + name, max(occ_lists, default = 0), sum(occ_lists), occ_lists)
            print()

        df_dicts.append(df_dict)
        
        print('Writing Results to File ... ')
        pd.DataFrame.from_dict(df_dicts).to_csv('.\\cortado_core\\experiments\\subpattern_eval\\Eval_Res\\' + log_name  + '_'+ strategy_name +'_Pattern_Mining_Memory.csv')   
        
        
if __name__ == "__main__":
    
    for strat, strat_name in [(FrequencyCountingStrategy.TraceOccurence, 'TraceOccurence'), (FrequencyCountingStrategy.VariantOccurence, 'VariantOccurence')]:
        ...
        for file in files:
            ...
            compare_occurence_list_size(log, log_name, strat, strat_name, timeout, False)